...
async def test_provider_selects_store_dynamically():
    ...
    await layer.maybe_substitute("frida", "t", _Result(json.dumps([{"x": 1}])), substitute=False)
    assert len(a.recent()) == 1 and len(b.recent()) == 0
    current["store"] = b
    await layer.maybe_substitute("frida", "t", _Result(json.dumps([{"y": 2}])), substitute=False)
    assert len(b.recent()) == 1


async def test_none_store_passes_through_without_capture():
    layer = CaptureLayer(inline_budget=10, launch_ts=1.0, store_provider=lambda: None)
    r = _Result(json.dumps([{"x": 1}]))
    assert await layer.maybe_substitute("frida", "t", r, substitute=True) is r


async def test_positional_store_still_works():
