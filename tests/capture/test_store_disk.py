...
async def test_large_body_spills_to_blob_and_get_restores_it(tmp_path):
    store = CaptureStore.open(tmp_path / ".pare" / "capture.db")
    ref = await store.write(_big_rec())
...
async def test_disk_permissions_are_hardened(tmp_path):
